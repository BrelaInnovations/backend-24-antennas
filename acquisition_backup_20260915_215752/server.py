...
if os.environ.get('NAIBRA_STORAGE', 'mongo').lower() == 'sqlite':
    from local_database import LocalDatabase
    client = LocalDatabase(os.environ.get('NAIBRA_DB_PATH', str(ROOT_DIR / 'data' / 'desktop.sqlite3')))
    db = client
else:
    from motor.motor_asyncio import AsyncIOMotorClient
    client = AsyncIOMotorClient(os.environ.get('MONGO_URL', 'mongodb://localhost:27017'), serverSelectionTimeoutMS=5000)
    db = client[os.environ.get('DB_NAME', 'naibra')]
...
class TruePosition(BaseModel):
    x: float = Field(allow_inf_nan=False)
    y: float = Field(allow_inf_nan=False)
    z: float = Field(ge=0, allow_inf_nan=False)

    @model_validator(mode="after")
    def inside_dome(self):
        from core.geometry import DOME_RADIUS_CM
        if math.hypot(self.x, self.y, self.z) > DOME_RADIUS_CM + 1e-9:
            raise ValueError(f"True coordinates must be inside the {DOME_RADIUS_CM:g} cm dome")
        return self


class ScanStart(BaseModel):
    label: Optional[str] = None
    true_position_cm: Optional[TruePosition] = None
...
async def start_scan(body: ScanStart):
    ...
    transport = cfg.get("transport", "wired" if cfg.get("connection_mode") == "live" else "simulated")
    simulated = transport == "simulated"
    hardware_capture = None
    if simulated:
        rng = random.Random()
        left = simulate_side(cfg, rng)
        right = simulate_side(cfg, rng)
        for side in (left, right):
            side["source"] = "simulated"
            side["dot_count"] = len(side["dots"])
            for dot in side["dots"]:
                dot["source"] = "simulated"
    elif transport in ("wired", "bluetooth"):
        # A failed hardware connection never falls back to simulated data.
        if scan_lock.locked():
            raise HTTPException(409, "A hardware scan is already running")
        async with scan_lock:
            hardware_capture = await run_in_threadpool(capture_real_hardware_sweep, cfg)
        if not hardware_capture:
            raise HTTPException(503, f"No hardware capture available for {transport}. Connect a supported scanner or select Simulated twin.")
        ...
        raise HTTPException(422, "Select simulated, wired, or bluetooth transport")
    if not simulated:
        right = {**left, "dots": list(left["dots"])}
    previous = None
    if simulated:
        previous = await db.scans.find_one({"simulated": True}, {"_id": 0}, sort=[("created_at", -1)])
        if previous and previous.get("metrics", {}).get("overall_score") is None:
            previous = None
    metrics = compute_metrics(left, right, previous) if simulated else localization_metrics(left, right)
    ants = antenna_positions(cfg) if simulated else localization_antennas()
    ...
    scan = {
        ...
        "display_mode": "simulated" if simulated else "localization",
        "algorithm": "simulated" if simulated else "dmas-cf",
        "simulated": simulated,
        "capture_scope": "simulated_bilateral" if simulated else "shared",
        "config_snapshot": {
    ...
    # Reference annotation only: never passed to acquisition or reconstruction.
    reference = getattr(body, "true_position_cm", None)
    if reference is not None:
        from core.geometry import DOME_RADIUS_CM
        scan["true_position_cm"] = reference.model_dump()
        scan["true_position_normalized"] = {k: v / DOME_RADIUS_CM for k, v in reference.model_dump().items()}
        peak = left.get("peak_location_cm")
        scan["localization_error_cm"] = (
            round(math.dist([peak[k] for k in ("x", "y", "z")], [reference.x, reference.y, reference.z]), 3)
            if peak else None
        )
...
async def compare_scans(scan_a: str, scan_b: str):
    ...
    if a["created_at"] > b["created_at"]:
        a, b = b, a
    deltas = {}
    if a["metrics"].get("overall_score") is not None and b["metrics"].get("overall_score") is not None:
        deltas = {
            "overall": b["metrics"]["overall_score"] - a["metrics"]["overall_score"],
            "left": b["left"]["score"] - a["left"]["score"],
            "right": b["right"]["score"] - a["right"]["score"],
            "asymmetry": b["metrics"]["left_right"]["asymmetry"] - a["metrics"]["left_right"]["asymmetry"],
            "high_dots_left": b["left"].get("dot_counts", {}).get("high", 0) - a["left"].get("dot_counts", {}).get("high", 0),
            "high_dots_right": b["right"].get("dot_counts", {}).get("high", 0) - a["right"].get("dot_counts", {}).get("high", 0),
        }
    return {

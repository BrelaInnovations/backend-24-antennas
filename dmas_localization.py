"""Convert DMAS-CF voxels to location markers, without severity scoring."""
import math
...
def localization_result(voxels, confident, dome_radius_cm=DOME_RADIUS_CM,
                        threshold_fraction=0.5, max_dots=4096):
    """Preserve voxel coordinates; threshold selects the displayed field only.

    Raw intensity is retained for inspection, never interpreted as severity.
    ...
    selected = [v for v in valid if v["intensity"] >= peak["intensity"] * threshold_fraction] if confident and peak else []
    ...
    dots = [{"x": v["x"] / dome_radius_cm,
             ...
             "intensity": v["intensity"], "source": "dmas-cf"} for v in selected]
    status = "localized" if dots else "inconclusive" if valid else "unavailable"
    return {
        ...
        "anomaly_count": None, "dot_counts": {}, "region_counts": {}, "worst_region": None,
    }
...
def localization_metrics(left, right):
    ...
    return {
        ...
        "recommendations": ["Dots show reconstructed locations. Both views share one physical capture."],
    }
